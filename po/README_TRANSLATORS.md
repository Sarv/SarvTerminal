# Localizing Sarv Terminal: The Translators' Guide

First of all, thanks for helping us localize Sarv Terminal!
...
Full locale names are more complicated, but Sarv Terminal does not use all parts. [The
...
template is generated automatically from Sarv Terminal's code and resources, and is
...
Afterwards, you need to update the list of known locales within Sarv Terminal's
...
You can then open a pull request on
[Sarv/SarvTerminal](https://github.com/Sarv/SarvTerminal) with the new
translation file.
...
Simply run `zig build run`. Sarv Terminal uses your system language by default; if
...
On some desktop environments, such as KDE Plasma, Sarv Terminal uses server-side
decorations by default. This hides many strings from the UI, which is
undesirable when viewing your translations. You can force Sarv Terminal to use
...
If you introduce a new language, please **consider volunteering to maintain
it** — mention it in your pull request description. Locale maintainers review
contributions to their locale's translations and help translate new strings
when the template changes. Ideally, a pull request adding a new language gets
a review from at least one other speaker of that language.
...
- **Make Sarv Terminal fit in with surrounding applications.**
  ...
  interface guidelines of each platform Sarv Terminal is available for, including the
