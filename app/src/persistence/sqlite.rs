...
fn handle_model_event(event: ModelEvent, connection: &mut SqliteConnection) -> anyhow::Result<()> {
    match event {
        ...
        ModelEvent::CreateSshGroup { name, label } => {
            crate::ssh_manager::db::create_group(connection, &name, &label, None)
                .map(|_| ())
                .context("error creating SSH group")
        }
        ModelEvent::DeleteSshGroup { id } => {
            crate::ssh_manager::db::delete_group(connection, id)
                .context("error deleting SSH group")
        }
        ModelEvent::RenameSshGroup { id, new_name } => {
            crate::ssh_manager::db::rename_group(connection, id, &new_name)
                .context("error renaming SSH group")
        }
        ModelEvent::CreateSshHost {
            group_id,
            alias,
            host,
            port,
            ...
            notes,
            label,
        } => crate::ssh_manager::db::create_host(
            connection,
            group_id,
            &alias,
            &host,
        ...
        .map(|_| ())
        .context("error creating SSH host"),
        ModelEvent::UpdateSshHost {
            id,
            group_id,
            alias,
            host,
            ...
            notes,
            label,
        } => crate::ssh_manager::db::update_host(
            connection,
            id,
            group_id,
            &alias,
        ...
        .context("error updating SSH host"),
        ModelEvent::DeleteSshHost { id } => {
            crate::ssh_manager::db::delete_host(connection, id)
                .context("error deleting SSH host")
        }
        ModelEvent::UpsertSyncConfig { repo_url, pat, master_pass } => {
            crate::sync_manager::db::upsert_config(connection, &repo_url, &pat, &master_pass)
                .context("error saving sync config")
        }
        ModelEvent::UpdateSyncResult { direction, error } => {
            crate::sync_manager::db::update_sync_result(connection, &direction, error.as_deref())
                .context("error updating sync result")
        }
    }
}
