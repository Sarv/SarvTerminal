...
/// Pass `overlay_icon` to show a custom icon on top of the dot.
...
pub(crate) fn render_color_dot(
    ...
    render_color_dot_inner(
        mouse_state,
        dot_color,
        is_selected,
        ring_color,
        if is_no_color { Some(Icon::SlashCircle) } else { None },
        foreground_color,
        tooltip_text,
        appearance,
    )
...
/// Like `render_color_dot` but accepts an explicit overlay icon instead of `is_no_color`.
#[allow(clippy::too_many_arguments)]
pub(crate) fn render_color_dot_with_icon(
    mouse_state: MouseStateHandle,
    dot_color: ColorU,
    is_selected: bool,
    ring_color: ColorU,
    overlay_icon: Option<Icon>,
    foreground_color: ThemeFill,
    tooltip_text: String,
    appearance: &Appearance,
) -> Hoverable {
...
fn render_color_dot_inner(
    ...
    Hoverable::new(mouse_state, move |state| {
        let overlay: Option<Box<dyn Element>> = overlay_icon
            .map(|icon| icon.to_warpui_icon(foreground_color).finish());
