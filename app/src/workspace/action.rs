...
pub enum WorkspaceAction {
    ...
    OpenNetworkLogPane,
    /// Toggle the SSH Manager panel for saved SSH host connections.
    ToggleSshManager,
    /// Open a new terminal tab running SSH to a saved host.
    ConnectSshHost {
        alias: String,
        host: String,
        port: i32,
    ...
    /// Toggle the Sync Settings panel.
    ToggleSyncManager,
    /// Manually push local settings to the remote git repo.
    SyncPushToRemote,
    /// Manually pull settings from the remote git repo.
    SyncPullFromRemote,
    /// Debounced auto-sync trigger (SSH host or settings change).
    TriggerAutoSync,
    /// Open the hex color input overlay for the given tab.
    PromptTabHexColor { tab_index: usize },
    /// Apply a custom RGB color to a tab (from the hex color input).
    SetTabCustomColor { r: u8, g: u8, b: u8, tab_index: usize },
}
