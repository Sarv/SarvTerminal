...
pub struct WorkspaceState {
    ...
    tab_being_hex_colored: Option<usize>, // The index of the tab whose color is being entered
...
impl WorkspaceState {
    ...
    pub fn close_all_modals(&mut self) {
        ...
        self.is_sync_manager_open = false;
    }
    ...
    pub fn is_tab_being_hex_colored(&self) -> bool {
        self.tab_being_hex_colored.is_some()
    }

    pub fn set_tab_being_hex_colored(&mut self, index: usize) {
        self.tab_being_hex_colored = Some(index);
        self.tab_being_renamed = None;
    }

    pub fn clear_tab_being_hex_colored(&mut self) {
        self.tab_being_hex_colored = None;
    }

    pub fn tab_being_hex_colored(&self) -> Option<usize> {
        self.tab_being_hex_colored
    }
...
fn get_terminal_background_opacity(window_id: WindowId, app: &AppContext) -> u8 {
    ...
    if let Some(img) = theme.background_image() {
    ...
    } else if BackgroundSettings::as_ref(app)
        .image_path
        .value()
        .as_deref()
        ...
        .unwrap_or(false)
    {
        // Terminal fill is independent of image opacity — the slider controls only
        // the image layer brightness. A solid Rect base in the workspace render
        // prevents any desktop bleed regardless of terminal or image opacity.
        background_opacity
    } else {
        background_opacity
    }
