...
use crate::ui_components::color_dot::{render_color_dot, render_color_dot_with_icon, TAB_COLOR_OPTIONS};
...
pub enum SelectedTabColor {
    ...
    Cleared,
    /// User explicitly chose this ANSI preset color.
    Color(AnsiColorIdentifier),
    /// User chose a custom RGB color via the color picker.
    Custom { r: u8, g: u8, b: u8 },
}

impl SelectedTabColor {
    /// Resolves to an ANSI identifier if one is set (returns None for Custom).
    pub(crate) fn resolve(
        ...
        match self {
            ...
            SelectedTabColor::Custom { .. } => None,
            SelectedTabColor::Cleared => None,
            SelectedTabColor::Unset => default,
        }
    }

    /// Resolves to a raw ColorU, handling both ANSI presets and custom RGB.
    pub(crate) fn resolve_color_u(
        self,
        default: Option<AnsiColorIdentifier>,
        normal_colors: &warp_core::ui::theme::AnsiColors,
    ) -> Option<warpui::color::ColorU> {
        match self {
            SelectedTabColor::Custom { r, g, b } => {
                Some(warpui::color::ColorU { r, g, b, a: 255 })
            }
            SelectedTabColor::Color(c) => {
                Some(c.to_ansi_color(normal_colors).into())
            }
            SelectedTabColor::Cleared => None,
            SelectedTabColor::Unset => default.map(|c| c.to_ansi_color(normal_colors).into()),
        }
    }
}
...
impl TabData {
    ...
    /// The resolved tab color as an ANSI identifier (None for custom RGB colors).
    pub fn color(&self) -> Option<AnsiColorIdentifier> {
        self.selected_color.resolve(self.default_directory_color)
    }

    /// The resolved tab color as a raw ColorU, handling both preset and custom colors.
    pub fn resolved_color_u(&self, normal_colors: &warp_core::ui::theme::AnsiColors) -> Option<warpui::color::ColorU> {
        self.selected_color.resolve_color_u(self.default_directory_color, normal_colors)
    }
    ...
    fn dot_color_option_menu_items(
        ...
        let is_custom = matches!(self.selected_color, SelectedTabColor::Custom { .. });
        let custom_color: Option<ColorU> = if let SelectedTabColor::Custom { r, g, b } = self.selected_color {
        ...
        let mouse_states: Vec<MouseStateHandle> = (0..TAB_COLOR_OPTIONS.len() + 1)
            ...
            .collect();
        let custom_mouse_state = MouseStateHandle::default();
        ...
        vec![MenuItem::Item(
            MenuItemFields::new_with_custom_label(
                Arc::new(move |_is_selected, _is_hovered, appearance, _app| {
                    ...
                    // Custom color dot — shows the active custom color when set, otherwise a
                    // gray dot with a + icon to invite picking a custom color.
                    let custom_dot_color = custom_color
                        .unwrap_or(ColorU { r: 90, g: 90, b: 90, a: 255 });
                    let custom_overlay = if is_custom { None } else { Some(Icon::Plus) };
                    let custom_icon_color = ThemeFill::from(ColorU { r: 220, g: 220, b: 220, a: 255 });
                    let custom_dot = render_color_dot_with_icon(
                        custom_mouse_state.clone(),
                        custom_dot_color,
                        is_custom,
                        ring_color,
                        custom_overlay,
                    ...
                    .on_click(move |ctx, _, _| {
                        ctx.dispatch_typed_action(WorkspaceAction::PromptTabHexColor {
                            tab_index: index,
                        });
                        ctx.dispatch_typed_action(MenuAction::Close(true));
                    });
                    row.add_child(custom_dot.finish());
...
pub struct TabBarState {
    ...
    pub is_any_tab_hex_coloring: bool,
...
impl TabStyles {
    ...
    fn default(appearance: &Appearance, tab_color: Option<ColorU>) -> TabStyles {
        let theme = appearance.theme();
        let active_tab_bar_color: Option<ThemeFill> = tab_color.map(ThemeFill::from);
...
impl<'a> TabComponent<'a> {
    pub fn new(
        ...
        Self {
            ...
            tab_index,
            styles: TabStyles::default(
                appearance,
                tab.resolved_color_u(&appearance.theme().terminal_colors().normal),
            ),
    ...
    fn is_tab_being_hex_colored(&self) -> bool {
        self.tab_bar.is_any_tab_hex_coloring && self.is_active_tab()
    }
    ...
    fn render_tab_content(&self) -> Box<dyn Element> {
        ...
        let inline_editor = if self.is_tab_being_renamed() {
            Some(self.editor.clone())
        } else if self.is_tab_being_hex_colored() {
            Some(self.hex_editor.clone())
        } else {
            None
        };
...
impl UiComponent for TabComponent<'_> {
    ...
    fn build(self) -> Self::ElementType {
        ...
        let tab_index = self.tab_index;
        let is_tab_being_renamed = self.is_tab_being_renamed() || self.is_tab_being_hex_colored();
        let is_last_tab = self.tab_bar.tab_count == 1;
