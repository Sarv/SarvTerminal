...
/// A sidebar navigation item: either a direct page link, a collapsible umbrella,
/// or an action button that dispatches a workspace action without navigating to a settings page.
pub enum SettingsNavItem {
    ...
    /// A button that dispatches a workspace action (used for local-mode tools).
    ActionButton {
        label: &'static str,
...
impl SettingsNavItem {
    pub fn render_action_button(&self, appearance: &Appearance, is_active: bool) -> Option<Hoverable> {
        if let SettingsNavItem::ActionButton { label, state } = self {
            let variant = if is_active { ButtonVariant::Accent } else { ButtonVariant::Text };
            let hoverable = appearance
                .ui_builder()
                .button(variant, state.clone())
