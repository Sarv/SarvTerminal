...
use warpui::platform::{Cursor, FilePickerConfiguration, FileType, GraphicsBackend, SystemTheme};
...
use crate::settings::{
    active_theme_kind, respect_system_theme, AIFontName, AppEditorSettings, BackgroundFit,
    BackgroundPosition, BackgroundSettings, CursorBlink, CursorBlinkEnabled, CursorDisplayType,
...
pub enum AppearancePageAction {
    ...
    BackgroundImageOpacitySliderDragged(f32),
    SetBackgroundImageFit(BackgroundFit),
    SetBackgroundImagePosition(BackgroundPosition),
}
...
pub struct AppearanceSettingsPageView {
    ...
    browse_background_button_mouse_state: MouseStateHandle,
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            BrowseBackgroundImage => {
                ctx.open_file_picker(
                    |result, ctx| {
                        if let Ok(paths) = result {
                            if let Some(path) = paths.first() {
                                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                                    report_if_error!(
                                        settings.image_path.set_value(Some(path.clone()), ctx)
                                    );
                                });
                                ctx.notify();
                            }
                    ...
                    FilePickerConfiguration::new()
                        .set_allowed_file_types(vec![FileType::Image]),
                );
            }
            SetBackgroundImagePath(path) => {
                let path = path.clone();
                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.image_path.set_value(Some(path), ctx));
                });
                ctx.notify();
            }
            ClearBackgroundImage => {
                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.image_path.set_value(None, ctx));
                });
                ctx.notify();
            }
            SetBackgroundImageOpacity(value) => {
                let opacity = value.round().clamp(0., 100.) as u8;
                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.image_opacity.set_value(opacity, ctx));
                });
                ctx.notify();
            }
            BackgroundImageOpacitySliderDragged(value) => {
                let opacity = value.round().clamp(0., 100.) as u8;
                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.image_opacity.set_value(opacity, ctx));
                });
            ...
            SetBackgroundImageFit(fit) => {
                let fit = *fit;
                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.image_fit.set_value(fit, ctx));
                });
                ctx.notify();
            }
            SetBackgroundImagePosition(pos) => {
                let pos = *pos;
                BackgroundSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.image_position.set_value(pos, ctx));
                });
                ctx.notify();
            }
        }
...
impl AppearanceSettingsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<AppearanceSettingsPageView>) -> Self {
        ...
        AppearanceSettingsPageView {
            ...
            browse_background_button_mouse_state: MouseStateHandle::default(),
    ...
    fn build_page(ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        categories.push(Category::new(
            "Terminal Background",
            vec![Box::new(BackgroundImageWidget::new())],
        ));
...
#[derive(Default)]
struct BackgroundImageWidget {
    opacity_slider_state: SliderStateHandle,
    clear_mouse_state: MouseStateHandle,
    fit_mouse_states: [MouseStateHandle; 4],
    position_mouse_states: [MouseStateHandle; 9],
}
...
impl BackgroundImageWidget {
    fn new() -> Self {
        Self {
            ...
            fit_mouse_states: std::array::from_fn(|_| MouseStateHandle::default()),
            position_mouse_states: std::array::from_fn(|_| MouseStateHandle::default()),
        }
    }
}

impl SettingsWidget for BackgroundImageWidget {
    type View = AppearanceSettingsPageView;

    fn search_terms(&self) -> &str {
        "background image wallpaper opacity fit cover contain stretch"
    }
    ...
    fn render(
        ...
        let bg_settings = BackgroundSettings::as_ref(app);
        let current_path = bg_settings.image_path.value().clone();
        let opacity = *bg_settings.image_opacity.value();
        let fit = *bg_settings.image_fit.value();
        let position = *bg_settings.image_position.value();
        let theme = appearance.theme();
        let font_size = appearance.ui_font_size();
        let font_family = appearance.ui_font_family();

        // Path row: truncated path text + Browse + Clear
        let path_label = current_path
            .as_deref()
            .unwrap_or("No image selected")
            .to_string();
        let path_text = Text::new_inline(path_label, font_family, font_size * 0.875)
            .with_color(if current_path.is_some() {
        ...
        let browse_btn = appearance
            .ui_builder()
            .button(ButtonVariant::Secondary, view.browse_background_button_mouse_state.clone())
            .with_style(UiComponentStyles {
                margin: Some(Coords::default().left(8.)),
                font_size: Some(font_size * 0.875),
                ..Default::default()
            })
            .with_text_label("Browse…".to_string())
            .build()
            .on_click(|ctx, _, _| {
                ctx.dispatch_typed_action(AppearancePageAction::BrowseBackgroundImage);
            })
            .finish();
        ...
        if current_path.is_some() {
            let clear_btn = appearance
                .ui_builder()
                .button(ButtonVariant::Error, self.clear_mouse_state.clone())
                .with_style(UiComponentStyles {
                    margin: Some(Coords::default().left(8.)),
                    font_size: Some(font_size * 0.875),
                    ..Default::default()
                })
                .with_text_label("Clear".to_string())
                .build()
                .on_click(|ctx, _, _| {
                    ctx.dispatch_typed_action(AppearancePageAction::ClearBackgroundImage);
                })
                .finish();
        ...
        let opacity_slider = appearance
            .ui_builder()
            .slider(self.opacity_slider_state.clone())
            .with_range(0_f32..100_f32)
            .with_default_value(opacity as f32)
            .with_style(UiComponentStyles {
                width: Some(160_f32.into()),
                margin: Some(Coords::default().top(3.).bottom(3.)),
                ..Default::default()
            })
            .on_drag(|ctx, _, val| {
                ctx.dispatch_typed_action(
                    AppearancePageAction::BackgroundImageOpacitySliderDragged(val),
                )
            })
            .on_change(|ctx, _, val| {
                ctx.dispatch_typed_action(AppearancePageAction::SetBackgroundImageOpacity(val))
            })
            .build()
            .finish();

        // Fit mode row
        // Cover = fill terminal, preserve aspect ratio (may crop)
        // Contain = fit within terminal, preserve aspect ratio
        // Stretch = fill terminal, ignore aspect ratio
        // Original = display at original size, no scaling
        let fit_options: [(&str, BackgroundFit); 4] = [
            ("Cover", BackgroundFit::Cover),
            ("Contain", BackgroundFit::Contain),
            ("Stretch", BackgroundFit::Stretch),
            ("Original", BackgroundFit::Original),
        ];
        let mut fit_row = Flex::row().with_cross_axis_alignment(CrossAxisAlignment::Center);
        for (i, (label, mode)) in fit_options.into_iter().enumerate() {
            let is_selected = fit == mode;
            let btn = appearance
                .ui_builder()
                .button(
                    if is_selected { ButtonVariant::Accent } else { ButtonVariant::Secondary },
                    self.fit_mouse_states[i].clone(),
                )
                .with_style(UiComponentStyles {
                    margin: Some(Coords::default().right(6.)),
                    font_size: Some(font_size * 0.875),
                    ..Default::default()
                })
                ...
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(AppearancePageAction::SetBackgroundImageFit(mode));
                })
                .finish();
            fit_row.add_child(btn);
        }

        // 3×3 position grid
        let pos_grid_cells: [(BackgroundPosition, &str); 9] = [
            (BackgroundPosition::TopLeft, "↖"),
            (BackgroundPosition::TopCenter, "↑"),
            (BackgroundPosition::TopRight, "↗"),
            (BackgroundPosition::CenterLeft, "←"),
            (BackgroundPosition::Center, "·"),
            (BackgroundPosition::CenterRight, "→"),
            (BackgroundPosition::BottomLeft, "↙"),
            (BackgroundPosition::BottomCenter, "↓"),
            (BackgroundPosition::BottomRight, "↘"),
        ];
        let cell_size = font_size * 2.;
        let mut pos_grid = Flex::column();
        for row_start in [0usize, 3, 6] {
            let mut row = Flex::row();
            for (idx, (pos, symbol)) in pos_grid_cells[row_start..row_start + 3].iter().enumerate() {
                let is_sel = *pos == position;
                let pos_val = *pos;
                let cell = appearance
                    .ui_builder()
                    .button(
                        if is_sel { ButtonVariant::Accent } else { ButtonVariant::Outlined },
                        self.position_mouse_states[row_start + idx].clone(),
                    )
                    .with_style(UiComponentStyles {
                        width: Some(cell_size.into()),
                        height: Some(cell_size.into()),
                        margin: Some(Coords::default().right(2.).bottom(2.)),
                        font_size: Some(font_size),
                        ..Default::default()
                    })
                    .with_text_label((*symbol).to_string())
                    .build()
                    .on_click(move |ctx, _, _| {
                        ctx.dispatch_typed_action(
                            AppearancePageAction::SetBackgroundImagePosition(pos_val),
                        );
                    })
                    .finish();
                row.add_child(cell);
            }
            pos_grid.add_child(row.finish());
        }
        ...
        Flex::column()
            ...
            .with_child(render_body_item::<AppearancePageAction>(
                ...
                appearance,
                fit_row.finish(),
                Some("Cover fills terminal preserving aspect ratio. Contain fits within. Stretch fills without preserving ratio. Original uses the image's native size.".to_string()),
            ))
            .with_child(render_body_item::<AppearancePageAction>(
                "Position:".to_string(),
                None,
