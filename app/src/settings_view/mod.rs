...
use crate::ssh_manager::manager::{SshManager, SshManagerEvent};
use crate::sync_manager::manager::{SyncManager, SyncManagerEvent, SyncStatus};
...
pub mod flags {
    ...
    pub const FEEDBACK_BUNDLED_SKILL_FLAG: &str = "Feedback_Bundled_Skill";
...
pub enum SettingsAction {
    ...
    /// Show a custom (non-SettingsSection) panel in the content area.
    ShowCustomPanel(&'static str),
}
    ...
    F: Fn(SettingsSection) -> bool,
{
    nav_items
        ...
        .flat_map(|(nav_index, item)| match item {
            ...
            SettingsNavItem::ActionButton { .. } => vec![],
        })
        .collect()
}
...
pub struct SettingsView {
    ...
    /// Embedded SSH Manager view for showing as a settings page.
    ssh_manager_handle: ViewHandle<SshManager>,
    /// Embedded Sync Manager view for showing as a settings page.
    sync_manager_handle: ViewHandle<SyncManager>,
    /// Which custom panel is currently showing in the content area (None = normal page).
    custom_panel: Option<&'static str>,
}
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        if local_mode {
            // Remove items that require a Warp account or are Warp-cloud-specific.
            nav_items.retain(|item| !matches!(item,
            ...
            nav_items.retain(|item| !matches!(item,
                SettingsNavItem::Umbrella(u) if u.label == "Agents"
                    || u.label == "Code"
                    || u.label == "Cloud platform"
            ));
            // Insert local-mode tools after Keybindings.
            let keybindings_pos = nav_items.iter().position(|item| {
                matches!(item, SettingsNavItem::Page(SettingsSection::Keybindings))
            });
            let insert_at = keybindings_pos.map(|i| i + 1).unwrap_or(nav_items.len());
            nav_items.insert(
                insert_at,
                SettingsNavItem::ActionButton {
                    label: "Sync Settings",
        ...
        let ssh_manager_handle = ctx.add_typed_action_view(SshManager::new);
        ctx.subscribe_to_view(&ssh_manager_handle, |me, _, event, ctx| {
            me.handle_ssh_manager_page_event(event, ctx);
        });
        let sync_manager_handle = ctx.add_typed_action_view(SyncManager::new);
        ctx.subscribe_to_view(&sync_manager_handle, |me, _, event, ctx| {
            me.handle_sync_manager_page_event(event, ctx);
        });
    ...
    fn handle_ssh_manager_page_event(
        &mut self,
        event: &SshManagerEvent,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            SshManagerEvent::Close => {
                self.custom_panel = None;
                ctx.notify();
            }
            SshManagerEvent::Connect { alias, host, port, user, pass } => {
                ctx.dispatch_typed_action(&WorkspaceAction::ConnectSshHost {
                    alias: alias.clone(),
                    host: host.clone(),
                    port: *port,
                    user: user.clone(),
            ...
            SshManagerEvent::HostCreated { group_name, label, alias, host, port, user, pass, notes } => {
                #[cfg(feature = "local_fs")]
                {
                    ...
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        let group_id = crate::ssh_manager::db::get_or_create_group_by_name(
                            &mut conn,
                            group_name,
                        )
                        .map(|g| g.id)
                        .unwrap_or(1);
                        let _ = crate::ssh_manager::db::create_host(
                            &mut conn,
                            group_id,
                            alias,
                            host,
                        ...
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
                        let hosts =
                            crate::ssh_manager::db::list_all_hosts(&mut conn).unwrap_or_default();
                        let labels =
                            crate::ssh_manager::db::list_standalone_labels(&mut conn).unwrap_or_default();
                        self.ssh_manager_handle
                            .update(ctx, |m, ctx| m.on_open(groups, hosts, labels, ctx));
                    }
                }
                ctx.dispatch_typed_action(&WorkspaceAction::TriggerAutoSync);
            }
            SshManagerEvent::HostUpdated {
                id,
                group_name,
                label,
                alias,
                ...
                #[cfg(feature = "local_fs")]
                {
                    ...
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        ...
                        .map(|g| g.id)
                        .unwrap_or(1);
                        let _ = crate::ssh_manager::db::update_host(
                            &mut conn,
                            *id,
                            group_id,
                            alias,
            ...
            SshManagerEvent::HostDeleted(id) => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    let db_path = database_file_path_for_scope(&PersistenceScope::App);
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        let _ = crate::ssh_manager::db::delete_host(&mut conn, *id);
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
                        let hosts =
                            crate::ssh_manager::db::list_all_hosts(&mut conn).unwrap_or_default();
            ...
            SshManagerEvent::GroupDeleted(id) => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    let db_path = database_file_path_for_scope(&PersistenceScope::App);
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        let _ = crate::ssh_manager::db::delete_group(&mut conn, *id);
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
                        let hosts =
                            crate::ssh_manager::db::list_all_hosts(&mut conn).unwrap_or_default();
            ...
            SshManagerEvent::GroupRenamed { id, name } => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    let db_path = database_file_path_for_scope(&PersistenceScope::App);
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        let _ = crate::ssh_manager::db::rename_group(&mut conn, *id, name);
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
                        let hosts =
                            crate::ssh_manager::db::list_all_hosts(&mut conn).unwrap_or_default();
            ...
            SshManagerEvent::GroupCreated { name, parent_id } => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        ...
                        let _ = crate::ssh_manager::db::create_group(
                            &mut conn,
                            name.trim(),
                            &label,
                            *parent_id,
            ...
            SshManagerEvent::LabelRenamed { old_name, new_name } => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    let db_path = database_file_path_for_scope(&PersistenceScope::App);
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        let _ = crate::ssh_manager::db::rename_label_on_hosts(
                            &mut conn, old_name, new_name,
                        );
                        let _ = crate::ssh_manager::db::rename_standalone_label(
                            &mut conn, old_name, new_name,
                        );
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
            ...
            SshManagerEvent::LabelRemoved(label) => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        let _ =
                            crate::ssh_manager::db::remove_label_from_hosts(&mut conn, label);
                        let _ =
                            crate::ssh_manager::db::delete_standalone_label(&mut conn, label);
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
                        let hosts =
                            crate::ssh_manager::db::list_all_hosts(&mut conn).unwrap_or_default();
            ...
            SshManagerEvent::LabelsCreated(names) => {
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        for name in names {
                            let _ = crate::ssh_manager::db::create_standalone_label(&mut conn, name);
                        }
                        let groups =
                            crate::ssh_manager::db::list_groups(&mut conn).unwrap_or_default();
                        let hosts =
    ...
    fn handle_sync_manager_page_event(
        &mut self,
        event: &SyncManagerEvent,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            SyncManagerEvent::Close => {
                self.custom_panel = None;
                ctx.notify();
            }
            SyncManagerEvent::Save {
                repo_url,
                pat,
                master_pass,
                save_master_to_keychain: _,
            } => {
                // All three fields are encrypted and stored in SQLite.
                // The vault key in Keychain is the only way to decrypt them.
                #[cfg(feature = "local_fs")]
                {
                    use crate::persistence::{
                        database_file_path_for_scope, establish_rw_connection, PersistenceScope,
                    ...
                    let db_path = database_file_path_for_scope(&PersistenceScope::App);
                    if let Ok(mut conn) = establish_rw_connection(db_path.to_str().unwrap_or("")) {
                        if let Err(e) = crate::sync_manager::db::upsert_config(
                            &mut conn,
                            repo_url,
                            pat,
                            master_pass,
                        ) {
                            log::error!("sync: save config failed: {e}");
                        }
                    }
                }
                self.sync_manager_handle
                    .update(ctx, |m, ctx| m.set_status(SyncStatus::Idle, ctx));
            }
            SyncManagerEvent::Test { repo_url, pat } => {
                let repo = repo_url.clone();
                let token = pat.clone();
                let handle = self.sync_manager_handle.clone();
                ctx.spawn(
                    async move { crate::sync_manager::git_sync::test_connection(&repo, &token) },
                    move |_, result, ctx| {
                        handle.update(ctx, |m, ctx| {
                            let status = match result {
                                Ok(()) => SyncStatus::TestOk,
                                Err(e) => SyncStatus::TestFailed(e.to_string()),
                            };
                            m.set_status(status, ctx);
                        });
                    },
                );
            }
            SyncManagerEvent::PushToRemote => {
                self.run_sync_in_settings(crate::sync_manager::git_sync::SyncDirection::Push, ctx);
            }
            SyncManagerEvent::PullFromRemote => {
                self.run_sync_in_settings(crate::sync_manager::git_sync::SyncDirection::Pull, ctx);
            }
        }
    }

    fn run_sync_in_settings(
        &mut self,
        direction: crate::sync_manager::git_sync::SyncDirection,
        ctx: &mut ViewContext<Self>,
    ) {
        #[cfg(feature = "local_fs")]
        {
            ...
            let db_path = database_file_path_for_scope(&PersistenceScope::App);
            let repo_url = establish_ro_connection(db_path.to_str().unwrap_or(""))
                .ok()
                .and_then(|mut conn| {
                    crate::sync_manager::db::get_config(&mut conn)
                        .ok()
                        .flatten()
                        .map(|c| c.repo_url)
                })
            ...
            if repo_url.is_empty() {
                self.sync_manager_handle.update(ctx, |m, ctx| {
                    m.set_status(
                        SyncStatus::TestFailed(
                            "No repo URL saved. Fill in the fields above and hit Save first.".to_string(),
                        ),
                        ctx,
                    );
                });
            ...
            let handle = self.sync_manager_handle.clone();
            let dir_str = direction.as_str().to_string();
            ctx.spawn(
                async move {
                    match direction {
                        crate::sync_manager::git_sync::SyncDirection::Push => {
                            crate::sync_manager::git_sync::push(&repo_url)
                        }
                        crate::sync_manager::git_sync::SyncDirection::Pull => {
                            crate::sync_manager::git_sync::pull(&repo_url)
                        }
                    }
                },
                move |_, result, ctx| {
                    let status = match result {
                        Ok(()) => SyncStatus::LastSyncOk {
                            direction: dir_str,
                            when: chrono::Utc::now().format("%Y-%m-%d %H:%M UTC").to_string(),
                        },
                        Err(e) => SyncStatus::LastSyncFailed {
                            direction: dir_str,
                            error: e.to_string(),
                        },
                    };
                    handle.update(ctx, |m, ctx| m.set_status(status, ctx));
                },
            );
        }
    }
...
impl View for SettingsView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let (page, current_page_handle) = if let Some(panel) = self.custom_panel {
            let view: Box<dyn Element> = match panel {
                "SSH Manager" => ChildView::new(&self.ssh_manager_handle).finish(),
                "Sync Settings" => ChildView::new(&self.sync_manager_handle).finish(),
                _ => Empty::new().finish(),
            };
            (view, None)
        } else if settings_pages.is_empty() {
        ...
        for (nav_index, nav_item) in self.nav_items.iter().enumerate() {
            match nav_item {
                SettingsNavItem::Page(section) => {
                    ...
                    {
                        let page_active = self.custom_panel.is_none() && section == self.current_settings_page;
                        buttons.add_child(
                ...
                SettingsNavItem::ActionButton { label, .. } => {
                    let label = *label;
                    let is_active = self.custom_panel == Some(label);
                    if let Some(hoverable) =
                        nav_item.render_action_button(appearance, is_active)
                    {
                        buttons.add_child(
                            hoverable
                                .on_click(move |ctx, _, _| {
                                    ctx.dispatch_typed_action(
                                        SettingsAction::ShowCustomPanel(label),
                                    );
                                })
                                .finish(),
                        );
...
impl TypedActionView for SettingsView {
    ...
    fn handle_action(&mut self, action: &SettingsAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SettingsAction::ShowCustomPanel(label) => {
                self.custom_panel = Some(label);
                if *label == "SSH Manager" {
                    #[cfg(feature = "local_fs")]
                    {
                        use crate::persistence::{
                            database_file_path_for_scope, establish_ro_connection, PersistenceScope,
                        ...
                        let db_path = database_file_path_for_scope(&PersistenceScope::App);
                        if let Ok(mut conn) =
                            establish_ro_connection(db_path.to_str().unwrap_or(""))
                        {
                            let groups = crate::ssh_manager::db::list_groups(&mut conn)
                                .unwrap_or_default();
                            let hosts = crate::ssh_manager::db::list_all_hosts(&mut conn)
                                .unwrap_or_default();
                            let labels = crate::ssh_manager::db::list_standalone_labels(&mut conn)
                                .unwrap_or_default();
                            self.ssh_manager_handle
                                .update(ctx, |m, ctx| m.on_open(groups, hosts, labels, ctx));
                        }
                    }
                } else if *label == "Sync Settings" {
                    #[cfg(feature = "local_fs")]
                    {
                        use crate::persistence::{
                            database_file_path_for_scope, establish_ro_connection, PersistenceScope,
                        ...
                        let db_path = database_file_path_for_scope(&PersistenceScope::App);
                        let (repo_url, pat, master, last_dir, last_at, last_err) =
                            if let Ok(mut conn) =
                                establish_ro_connection(db_path.to_str().unwrap_or(""))
                            {
                                crate::sync_manager::db::get_decrypted_config(&mut conn)
                                    .ok()
                                    .flatten()
                                    .unwrap_or_default()
                            } else {
