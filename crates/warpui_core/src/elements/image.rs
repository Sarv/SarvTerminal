...
/// 9-position alignment grid for an image within its container.
...
impl Image {
    ...
    /// Sets the 9-position alignment of the image within its container.
    /// Overrides `top_aligned` and `right_aligned` when set.
...
fn image_rect(
    ...
    let offset = if let Some(align) = alignment {
        let cx = (size.x() - logical_image_size.x()) / 2.0;
        let rx = size.x() - logical_image_size.x();
        let cy = (size.y() - logical_image_size.y()) / 2.0;
        let by_ = size.y() - logical_image_size.y();
        match align {
            ImageAlignment::TopLeft => vec2f(0.0, 0.0),
            ImageAlignment::TopCenter => vec2f(cx, 0.0),
            ImageAlignment::TopRight => vec2f(rx, 0.0),
            ImageAlignment::CenterLeft => vec2f(0.0, cy),
            ImageAlignment::Center => vec2f(cx, cy),
            ImageAlignment::CenterRight => vec2f(rx, cy),
            ImageAlignment::BottomLeft => vec2f(0.0, by_),
            ImageAlignment::BottomCenter => vec2f(cx, by_),
            ImageAlignment::BottomRight => vec2f(rx, by_),
        }
