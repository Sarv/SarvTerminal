...
pub fn defaultDir(io: std.Io, alloc: Allocator, environ_map: *const std.process.Environ.Map) !Dir {
    // SarvTerminal divergence: isolate crash reports to our own state dir
    // instead of the shared `ghostty/crash` (see UPSTREAM.md).
    const crash_dir = try internal_os.xdg.state(io, alloc, environ_map, .{ .subdir = "sarvterminal/crash" });
