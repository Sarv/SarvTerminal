...
fn cacheDir(io: std.Io, alloc: Allocator, environ_map: *const std.process.Environ.Map) ![]const u8 {
    ...
    return try internal_os.xdg.cache(
        ...
        // (see UPSTREAM.md §8.4). macOS uses the NSCaches branch above,
        // scoped by build_config.bundle_id = com.sarv.terminal.
...
pub const Transport = struct {
    ...
    fn sendInternal(envelope: *sentry.Envelope) !void {
        ...
        const path = try std.fs.path.join(alloc, &.{
            state_dir,
            // SarvTerminal divergence: our own crash-report extension
            // (upstream uses `.ghosttycrash`). See UPSTREAM.md §8.4.
