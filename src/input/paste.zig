...
///
/// This is deliberately NOT upstream's `isSafeWith`, which relaxes the rule
/// based on bracketed-paste state. Ours is orthogonal: it stays conservative
/// about embedded newlines regardless of terminal mode, and only forgives the
/// trailing one. `Surface.zig` paste protection calls this.
