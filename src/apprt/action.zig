...
pub const Action = union(Key) {
    ...
    /// Reopen the most recently closed tab. If the target is a surface the
    /// tab should be restored into that surface's window when possible,
    /// otherwise the apprt may restore it into a new window.
    reopen_closed_tab,
