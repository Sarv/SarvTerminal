...
**Duplicate tab** (`duplicateTab`, tab context menu). A duplicate must be a faithful clone of its source: same name, working directory (the focused pane's live `pwd`, spawned as the new shell's cwd — never a typed `cd`), host, and **accent color**. An SSH tab (`launchCommand` starts with `ssh `) re-runs through the staged connection flow rather than cloning the live session. It lands immediately to the right of the tab it came from, at `sourceIndex + 1`.

**A new tab must be CREATED at its final index — never appended and then moved.** Every creation path (`newTerminal`, `newCommandTerminal`, `newSerial`, `startSSHConnection`, `openSavedSession`) routes through one helper, `place(_:at:)`, which inserts at a clamped index (or appends when `nil`) and then sets the selection. Reason: the tab strip scrolls the *selected* tab into view whenever the selection changes, so a tab that is momentarily last — even for a single UI update, and even when the subsequent move is animated — makes the strip scroll to the far end of the strip while the chip animates back to the middle. With a handful of tabs everything fits on screen and nothing shows; with 20+ tabs the user is thrown from the middle of the strip to the last tab on every duplicate. `detachPane` already inserted at the target index and was unaffected. A GTK port must mirror all of it: copy every visible attribute incl. color, insert next to the source, and keep a single insert-then-select entry point so no path can reintroduce a transient end position. Verify with 20+ tabs, scrolled to the middle: right-click → Duplicate Tab (once on a local tab, once on an SSH tab) — the duplicate appears beside its source, is focused, and the strip does not scroll away.
...
- **Per-window close:** the LAST window's red button quits the app (existing app-wide confirm-if-running); a SECONDARY window just closes, and its confirm checks only *that window's* sessions (`VaultsTabsModel.hasBusyProcess`) so a busy *other* window doesn't cause a prompt.
